...
public class DealsafariGR {
    public static final String USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:79.0) Gecko/20100101 Firefox/79.0";

    public void print( String city) throws IOException {
        final String cityquery = city;
        DatabaseMng db = new DatabaseMng();

        final Document page = Jsoup.connect("https://www.dealsafari.gr/prosfores/xenodoxeia?location=" + URLEncoder.encode(cityquery, "UTF-8")).userAgent(USER_AGENT).timeout(10 * 1000).get();

        Elements namesOfHotel = page.select("div.list-deal-company-name");
        Elements hotelPrice = page.select("span.list-deal-final-price");

        //Έλεγχος σύνσεση με Database1
        try {
        ...
        int sum = 0;
        System.out.println("--------------------------------------");
        System.out.println("Αποτελέσματα απο DealsafariGR");
